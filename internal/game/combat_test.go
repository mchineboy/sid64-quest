...
func TestPvECombatPersistenceAndSafety(t *testing.T) {
	...
	choir, inn := roomID("crypt_choir"), roomID("inn")
	...
	_, err = ws.Attack(ctx, player, inn, "sentinel")
	...
	require.Equal(t, int64(8), current.Gold)
	...
	// Exhaustion rejects the entire action, then defeat relocates without losses.
	gallery := roomID("mine_gallery")
	_, err = db.Exec(`UPDATE characters SET current_room_id=$2,health=1,stamina=1 WHERE id=$1`, player, gallery)
	require.NoError(t, err)
