...
Each invocation has a 50,000-step budget, a two-second wall deadline, a maximum of 16 messages of 512 bytes each, and rejects terminal control characters. There are at most four concurrent workers per gateway process. Workers receive only serialized event data, source and scoped state, with no database credentials in their environment. Linux additionally enforces a two-second CPU limit and disabled core dumps. Go's 128 MiB memory target is a soft target, not an allocation quota. A virtual-address-space limit is deliberately not used because Go reserves a large sparse arena on ARM64; bounding it causes false out-of-memory failures without bounding resident memory.
